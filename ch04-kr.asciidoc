...
비트코인 거래의 지불 부분에 있어서 비트코인 수신인의 공개키는 _비트코인 주소_ 라고불리는 디지털 지문으로 표현되며, 이것은 수표에 수취인의 이름을 적는 것 (즉, "~에게 지불합니다")과 유사한 방식이라고 생각하면 된다. 대부분의 경우 비트코인 주소는 공개키로부터 생성되고 공개키에 대응한다. 하지만 모든 비트코인 주소가 공개키를 표현하는 것은 아니다; 스크립트 등이 공개키 역학을 하기도 하며, 이후에 이 내용에 대해 다룰 것이다. 이러한 방식을 통해 비트코인 주소는 수표와 유사하게, 돈의 수취와 관련된 정보를 요약함으로써 송금처를 자유롭게 지정할 수 있다: 개인 계좌나 기업 계좌, 청구서에 대한 지불이나 현금 거래에 사용되는 단일 지급 수단으로 사용가능하다. 비트코인 주소는 사용자들이 일상적으로 보게 될 키를 대신하는 유일한 수단이며, 이는 타인과 공유해야 하는 부분이기 때문이다.
...
먼저 암호학과 비트코인에 사용되는 수학에 대해 다룰 것이다. 다음으로 키의 생성, 저장 및 관리에 대해 알아볼 것이다. 프라이빗 및 공개키, 주소와 스크립트 주소를 대신하기 위해 사용되는 다양한 인코딩 형식에 대해 살펴볼 것이다. 마지막으로 주소 꾸미기 (vanity), 다중서명 (multisignature) 및 스크립트 주소와 종이 지갑 등 키와 주소의 특별한 용도에 대해 다룰 것이다.
...
((("keys and addresses", "overview of", "public key cryptography")))((("digital currencies", "cryptocurrency")))공개키 암호화는 1970년대에 발명되었고 컴퓨터와 정보 보안의 수학적 토대이다.
...
공개키 암호화 발명이후로, 소수 지수 함수 및 타원곡선 곱셈과 같은 몇 가지 적합한 수학함수가 발견되었다. 이 수학 함수들은 실제로는 불가역성이다. 즉, 한 방향으로 계산하는 것은 쉽지만 그것을 역으로 계산하는 것은 불가능하다. 이러한 수학 함수에 기반하여, 암호화는 디지털 비밀번호와 위조 불가능한 디지털 서명을 생성할 수 있다. 비트코인은 암호화의 기본 토대로 타원곡선 곱셈을 사용한다.
...
비트코인에서는 비트코인에 접근하는 권한을 주는 한 쌍의 키를 생성하기 위해 공개키 암호화를 활용한다. 한 쌍의 키는 프라이빗키와--프라이빗키에서 파생된--고유의 공개키로 구성된다. 공개키는 비트코인을 전송받을 때 사용되며, 프라이빗키는 비트코인을 소비하기 위해 트랜잭션에 서명할 때 사용된다.
...
공개키와 프라이빗 간에는 수학적인 관계가 성립하며, 이 때문에 개인키를 메시지에 서명할 때 사용할 수 있다. 이 서명은 프라이빗키를 노출하지 않고 공개키만으로 유효성을 확인하는 것이 가능하다.
...
비트코인을 소비할 때, 현재 비트코인 소유자가 비트코인을 소비하기 위해 트랜잭션에서 공개키와 서명 (매번 다르지만 동일한 프라이빗키로 부터 생성됨)을 제시한다. 공개키와 서명을 제시함으로써, 비트코인 네트워크의 모든 사람이 트랜잭션을 유효한 거래로 검증하고 받아들일 수 있게된다. 동시에 비트코인을 전송하려는 사람이 전송 당시에 비트코인을 보유하고 있었는지 여부도 확인가능하다.
...
((("keys and addresses", "overview of", "key pairs")))대부분의 지갑 구현에서, 프라이빗키와 공개키는 편의성을 위해 _한 쌍의 키 (key pair)_ 형태로 저장된다. 그러나 공개키는 개인키를 기반으로 계산할 수 있기 때문에 프라이빗키만 저장하는 것도 가능하다.
====
...
((("keys and addresses", "overview of", "private and public key pairs")))((("elliptic curve cryptography")))((("cryptography", "elliptic curve cryptography")))비트코인 지갑은 프라이빗키와 공개키로 구성된 각각의 키 쌍을 포함한다. 프라이빗키 (k)는 일반적으로 무작위로 추출된 숫자이다. 프라이빗키로부터 일방향 타원곡선 곱셈을 사용하여 공개키 (K)를 생성한다. 공개키 (K)로부터 일방향 암호화 해시함수를 사용하여 비트코인 주소 (A)를 생성할 수 있다. 이제부터 우리는 프라이빗키를 생성하는 것을 시작으로 공개키로 전환하는데 사용하는 타원곡선 함수를 살펴보고, 마지막으로 공개키로부터 비트코인 주소를 생성할 것이다. 프라이빗키, 공개키와 비트코인 주소에 대한 관계는 <<k_to_K_to_A>>에 설명되어 있다.
...
((("cryptography", "asymmetric")))((("digital signatures", "asymmetric cryptography and")))((("asymmetric cryptography")))왜 비트코인에서 비대칭 암호화를 사용하는가? 트랜잭션을 "암호화 (encrypt)" (비밀 유지) 하기 위해 사용되는 것은 아니다. 오히려 비대칭 암호화의 유용한 특징은 _디지털 서명_ 을 생성하는 능력이다. 프라이빗키는 숫자로 된 서명을 생성하여 트랜잭션의 디지털 지문으로 사용될 수 있다. 이러한 서명은 프라이빗키에 대한 정보를 가지고 있는 사람만 생성할 수 있다. 그러나 공개키와 트랜잭션 지문에 접근 할 수 있는 모든 사람은 이것을 서명을 _검증하기 위해_ 사용할 수 있다. 이러한 비대칭 암호화의 특징은 누구나 각각 트랜잭션의 서명을 검증할 수 있게 하면서, 프라이빗키 소유자만이 유효한 서명을 생성할 수 있게 한다.
****
...
((("keys and addresses", "overview of", "private key generation")))((("warnings and cautions", "private key protection")))프라이빗키는 무작위로 선정된 숫자이다. 프라이빗키에 대한 소유권과 제어는 해당하는 비트코인 주소와 연관된 모든 자금에 대한 사용자 제어의 핵심이다. 프라이빗키는 서명을 생성하기 위해 사용된다. 이 서명은 트랜잭션에 사용되는 자금의 소유권을 증명하므로 비트코인을 소비할 때 요구된다. 프라이빗키를 공개하는 것은 프라이빗키로 보안된 비트코인에 대한 통제권을 넘겨주는 것과 동일하기 때문에 절대 공개되어서는 안된다. 또한 프라이빗키를 분실하면 복원할 수 없고 해당 프라이빗키로 보안된 자금도 영구적으로 손실되므로 프라이빗키는 항상 백업되어야 한다.
...
비트코인 프라이빗키는 단순한 숫자이다. 여러분은 단지 동전이나 연필, 종이를 가지고 무작위로 프라이빗키를 고를 수 있다: 동전을 256회 던져서 비트코인 지갑에서 사용할 수 있는 무작위 개인키를 구성하는 2진수를 결정하는 식이다. 그 후 공개키는 프라이빗키로부터 생성된다.
====
...
키를 생성하는데 있어서 가장 첫 단계이자 중요한 과정은 엔트로피의 안전한 출처, 또는 무작위성을 찾는 것이다. 비트코인 키를 생성하는 것은 본질적으로 "1에서 2^256^ 사이의 숫자를 선택하는 것"과 동일하다. 선택한 숫자가 예측 또는 재현 가능하지 않다면 숫자 선택시 정확히 어떤 방법을 사용했는지는 중요하지 않다. 비트코인 소프트웨어는 256비트의 엔트로피 (무작위성)을 생성하기 위해 기본적인 운영체제의 난수생성기를 이용한다. 일반적으로 OS의 난수생성기는 사람이 직접 무작위성을 만들어내므로, 몇 초 동안 마우스를 이리저리 움직여야 될 수도 있다.
...
더 정확하게 설명하자면, 프라이빗 키는 +0+ 와 +n - 1+ 사이의 숫자로 구성될 수 있다. n (n = 1.1578 * 10^77^, 2^256^ 보다 약간 작은 값)은 비트코인에 사용된 타원곡선의 위수 (order)라고 정의된 상수이다 (<<elliptic_curve>> 를 참고하라). 이 키를 생성하기 위해서, 무작위로 256비트의 숫자를 선택하고 이 숫자가 +n+ 보다 작은지 확인한다. 프로그래밍 측면에서 보면, 암호학적으로 안전한 무작위로부터 추출된 무작위 비트 문자열 중에서 크기가 큰 것들을 SHA256 해시 알고리즘에 집어넣으면 편리하게 256-비트 숫자를 생성할 수 있다. 결과값이 +n+ 보다 작다면, 알맞은 개인키가 생성된 것이다. 그렇지 않으면, 단순히 다른 난수를 사용하여 다시 시도해보면 된다.
...
((("random numbers", "random number generation")))((("entropy", "random number generation")))난수를 만들기 위해 자체 코드를 쓰거나 프로그래밍 언어게 제공하는 "단순한" 난수생성기를 사용해서는 안된다. 충분한 엔트로피를 가지고 있는 소스로부터 얻은 시드 (seed)를 이용해서 암호학적으로 안전한 유사난수생성기 (CSPRNG)를 사용하라. 당신이 선택한 난수생성기가 암호학적으로 안전한지 확인하기 위해 난수생성기 라이브러리에 대한 자료를 연구해야 한다. 키의 보안성에 있어 올바른 CSPRNG를 구현하는 것은 중요하다.
====
...
다음은 16진법 형식으로 표시한 (각 4비트씩, 64개의 16진수로 표현된 256비트) 무작위로 생성된 프라이빗키 (k)이다.
...
비트코인 프라이빗키 공간 크기 (2^256^)는 대단히 큰 숫자이다. 10진법으로 표현하면 대략 10^77^ 정도이다. 비교를 위해서 참고로 가시적인 우주는 10^80^ 개의 원자로 구성되어 있다고 추정된다.
====
...
((("dumpprivkey command")))비트코인 코어 클라이언트를 사용하여 새로운 키를 생성하려면 (<<ch03_bitcoin_client>> 를 참조하라), +getnewaddress+ 명령어를 사용하라.보안상의 이유로 공개키만 표시되고 프라이빗키는 표시되지 않는다. +bitcoind+ 에 개인키를 표시하도록 하려면, +dumpprivkey+ 명령어를 사용하라. +dumpprivkey+ 명령어는 _Wallet Import Format_ (WIF)인 Base58 체크섬으로 인코딩된 (checksum-encoded) 포맷으로 개인키를 표시한다. 이에 대해서는 <<priv_formats>> 에서 더 자세하게 살펴볼 것이다. 다음은 이 두가지 명령어를 사용하여 프라이빗키를 생성하고 표시하는 예이다.
...
+dumpprivkey+ 명령어는 지갑을 열고 +getnewaddress+ 명령어로 생성된 프라이빗키를 추출한다. 프라이빗키와 공개키가 지갑에 둘다 저장되어 있지 않은 경우에는 +bitcoind+ 에서 공개키로부터 프라이빗키를 알아내는 것은 불가능하다.
...
+dumpprivkey+ 명령어는 공개키로부터 프라이빗키를 생성하는 것이 불가능하는 작업이 불가능하기 때문에 프라이빗키를 생성하지 않는다. 이 명령어는 단순히 이미 지갑에 있고 +getnewaddress+ 명령어를 통해 생성된 프라이빗키만을 보여준다.
=====================================================================
...
또한 여러분은 비트코인 익스플로러 명령어 라인 도구 (command-line tool) (<<appdx_bx>> 참고)를 사용하여, +seed+, +ec-new+ 및 +ec-to-wif+ 명령어를 사용하여 프라이빗키를 생성하고 표시할 수 있다:
...
((("keys and addresses", "overview of", "public key calculation")))((("generator point")))공개키는 타원곡선 곱셈함수를 이용해서 프라이빗키로부터 계산되며, 이 과정은 비가역적이다: _K_ = _k_ * _G_ 이 식에서  _k_ 는 프라이빗키, _G_ 는 _생성 포인트 (generator point)_ 라고 하는 상수이며, _K_ 는 계산 결과로 나온 공개키이다. "이산로그 (discrete logarithm) 찾기"로도 알려진 역연산은 _K_ 값을 안다는 가정 하에 _k_ 값을 계산하는 과정인데, 가능한 _k_ 값을 모두 대입해보는 것만큼 어렵다 (즉, 무차별 대입공격). 프라이빗키로부터 공개키를 어떻게 생성하는가를 설명하기 전에, 타원곡선 암호법에 대해 더 자세히 살펴보자.
...
타원곡선 곱셈함수는 암호학자들이 "트랩 도어 (trap door)" 이라고 하는 함수 유형 중 하나이다: 단방향 (곱셈)의 연산은 쉽지만, 역연산 (나누기)은 불가능하다. 프라이빗키 소유자는 누군가 함수를 역연산하거나 공개키로부터 프라이빗키를 계산하는 것은 불가능하다는 것을 알고 있기 때문에, 쉽게 공개키를 생성하고 공유할 수 있다. 이 수학적 방법은 비트코인의 소유권을 증명하는 위조불가능하고 안전한 디지털 서명의 기초가 된다.
====
...
((("keys and addresses", "overview of", "elliptic curve cryptography")))((("elliptic curve cryptography", id="eliptic04")))((("cryptography", "elliptic curve cryptography", id="Celliptic04")))타원곡선 암호법은 비대칭 또는 공개키 암호법의 일종이다. 이 암호법은 이산로그 문제를 기반으로 하며 타원곡선 위의 점에서 덧셈이나 곱셈으로 표시된다.

// <<ecc-curve>> is an example of an elliptic curve, similar to that used by bitcoin.
<<ecc-curve>> 는 타원곡선의 한 예로, 바트코인에서 사용되는 것과 유사하다.
...
비트코인은 미국립표준기술원 (NIST)에서 개발된 +secp256k1+ 표준에 정의된 대로 특정 타원곡선과 수학 상수 집합을 사용한다. +secp256k1+ 곡선은 타원곡선을 생성하는 다음의 함수로 정의된다.
...
_mod p_ (modulo 소수 p)는 위 곡선이 소수위수 (prime order) _p_ 의 유한체 체제에 존재한다는 사실을 나타내며, latexmath:[\( \mathbb{F}_p \)] 로 표기되기도 한다. 여기에서 p = 2^256^ – 2^32^ – 2^9^ – 2^8^ – 2^7^ – 2^6^ – 2^4^ – 1 로 매우 큰 소수이다.
...
이 곡선이 실수 체제가 아닌 소수위수의 유한체 체제에서 정의되기 때문에, 2차원 그래프에 분포되어 있는 점들의 패턴이라 시각화하기가 어렵다. 그러나 사용되는 수학바식은 실수 체제의 곡선과 동일하다. 예를 들어, <<ecc-over-F17-math>> 는 소수위수가 17인 소규모 유한체에서 동일한 곡선을 나타내며, 격자 그래프 상의 점 패턴을 보여준다. +secp256k1+ 비트코인 타원곡선은 대단히 큰 격자그래프 위에 훨씬 더 복잡한 유형의 점 패턴이라고 생각하면 된다.

[[ecc-over-F17-math]]
[role="smallersixty"]
// .Elliptic curve cryptography: visualizing an elliptic curve over F(p), with p=17
.타원곡선 암호법: p=17인 경우 F(p)에 대한 타원곡선의 시각화
...
아래의 예시는 +secp256k1+ 위의 한 점 (x,y)로 이루어진 포인트 P이다.
...
<<example_4_1>> 에서는 파이썬을 사용하여 확인할 수 있는 방법을 보여준다.

[[example_4_1]]
// .Using Python to confirm that this point is on the elliptic curve
.파이썬을 사용하여 타원곡선 위의 점인지 확인하기
====
[source, pycon]
----
...
타원곡선 수학에서는 "무한원점 (point at infinity)"라는 점이 있는데, 이 점은 0의 역할을 한다고 생각하면 된다. 컴퓨터상에서는 x=y=0 으로 가끔 표현되기도 한다 (타원곡선 방정식을 만족하지는 않지만 쉽게 확인할 수 있는 별개의 방법이다).
...
또한 "덧셈 (addition)"이라고 하는 [+] 연산자도 있으며, 이것은 초등학교에서 배우는 실수의 일반적인 덧셈이 가지는 특성과 유사하다. 타원곡선 위의 두 점 P~1~ 와 P~2~ 가 주어졌을 때, 타원곡선 위에는 세 번째 점 P~3~ = P~1~ + P~2~ 도 존재한다.
...
기하학적으로 세 번째 점 P~3~ 은 P~1~ 와 P~2~ 사이에 직선을 그어서 계산한다. 이 선은 정확히 P~1~ 와 P~2~ 가 아닌 또 다른 한 점에서 곡선과 교차하게 된다. 이 점을 P~3~' = (x, y) 라 하자. 그 후 x축에 대칭시키면 P~3~ = (x, –y) 를 얻을 수 있다.

// There are a couple of special cases that explain the need for the "point at infinity."
"무한원점"의 개념이 필요한 경우도 몇 가지 존재한다.
...
P~1~ 와 P~2~ 가 동일한 점일때, P~1~ 와 P~2~ "사이에" 존재하는 직선은 P~1~ 위 곡선의 접선으로 연장된다. 이 접선은 곡선의 다른 한 점과 만나게 된다. 미적분을 사용하여 접선의 기울기를 구할 수 있다. 두 개의 정수로 구성된 곡선 상의 점에만 집중하고 있지만, 이 방법은 신기하게도 제 역할을 한다.
...
어떤 경우 (P~1~ and P~2~ 가 동일한 x값을 가지지만 y값은 다를 때)에는 접선은 정확히 수직이 되며, 이 경우 P3 = "무한원점" 이다.
...
P~1~ 이 "무한원점" 이라면, P~1~ + P~2~ = P~2~ 가 된다. 유사하게 P~2~ 이 무한원점이라면, P~1~ + P~2~ = P~1~ 이 된다. 이 경우가 무한원점이 0의 역할을 어떻게 수행하는지를 잘 보여준다.
...
[+] 는 결합성 (associative)을 가지고 있다. 이는 (A pass:[+] B) pass:[+] C = A pass:[+] (B pass:[+] C) 를 의미한다. 즉, 괄호없이 A A pass:[+] B pass:[+] C 라고 쓰더라도 전혀 모호하지 않다.
...
덧셈에 대해서 살펴보았고, 우리는 이제 덧셈까지 포괄할 수 있는 표준 방법으로 곱셈을 정의할 수 있다. 타원곡선 위의 점 P에 대해 k가 정수인 경우, kP = P + P + P + ... + P (k 배) 이다. 이 경우 때때로 k를 "지수 (exponent)"라고 불러서 혼동할 수 있으니 주의하자.
...
((("keys and addresses", "overview of", "public key generation")))((("generator point")))무작위로 생성된 숫자 _k_ 로 이루어진 프라이빗키를 출발점으로 하여, _k_ 에 _생성 포인트_ _G_ 를 곱하여 곡선상의 다른 곳에 위치한 다른 점을 생성할 수 있다. 이 점이 공개키인 _K_ 가 된다. 생성 포인트는 +secp256k1+ 표준의 일부로 명시되어 있고 비드코인의 모든 키에 대해 동일한 값을 가진다:
...
_k_ 는 프라이빗키, _G_ 는 생성 포인트, _K_ 는 곡선 위의 한 점으로 생성된 공개키이다. 모든 비트코인 사용자에 대해 생성 포인트는 항상 동일하므로, 프라이빗키 _k_ 에 _G_ 를 곱하게 되면, 항상 동일한 공개키 _K_ 가 나올 것이다. _k_ 와 _K_ 값의 관계는 고정되어 있지만, _k_ 에서 _K_ 로 한 방향으로의 계산만 가능하다. 그래서 비트코인 주소 (_K_ 값에서 파생)는 누구와도 공유할 수 있지만 사용자의 프라이빗키 (_k_)는 공개해서는 안된다.

[TIP]
====
// A private key can be converted into a public key, but a public key cannot be converted back into a private key because the math only works one way.
프라이빗키는 공개키로 전환될 수 있지만, 단방향 연산만 가능하므로 공개키를 프라이빗키로 다시 전환할 수는 없다.
====
...
타원곡선 곱셈 함수를 구현하기 위해서는 이전에 계산되어 나온 프라이빗키 _k_ 에 생성 포인트 G 값을 곱하여 공개키 _k_ 를 구하면 된다.
...
정수 좌표인 한 점의 곱셈을 시각화하기 위해 실수 체계에서 사용하는 더 간단한 타원곡선을 사용하게 될 것이다: 동일한 수학이 사용됨을 명심하라. 우리의 목적은 생성포인트 _G_ 의 배수 _kG_ 를 찾는 것이며, 이는 _G_ 를 연속하여 _k_ 번 더하는 것과 동일하다. 타원곡선에서 한 점을 연속하여 더하는 것은 그 점에서 접선을 그리는 것과 동일하고, 곡선과의 교차점을 찾은 후 x축에 대칭시킨다.

// <<ecc_illustrated>> shows the process for deriving _G_, _2G_, _4G_, as a geometric operation on the curve.
<<ecc_illustrated>> 는 곡선상에서 기하학적 방법을 통해 _G_, _2G_, _4G_ 를 구하는 과정이다.
...
((("OpenSSL cryptographic library")))대부분의 비트코인 구현들은 타원곡선 계산을 위해 http://bit.ly/1ql7bn8[OpenSSL cryptographic library] 를 사용한다. 예를 들어, 공개키를 얻기 위해서는 +EC_POINT_mul()+ 함수를 이용한다.((("", startref="KAover04")))
====

[[ecc_illustrated]]
// .Elliptic curve cryptography: visualizing the multiplication of a point G by an integer k on an elliptic curve
.타원곡선 암호법: 곡선상의 점 G와 정수 k의 곱을 시각화
...
((("keys and addresses", "bitcoin addresses", id="KAaddress04")))비트코인 주소는 숫자와 문자로 구성된 문자열로 당신에게 돈을 전송하고자 하는 어떤 누구와도 공유할 수 있다. 공개키로부터 생성된 주소는 숫자와 문자로 구성되며, 숫자 '1'로 시작한다. 다음은 비트코인 주소의 한 예이다.
...
비트코인 주소는 자금의 '수신자' 자격으로 거래에서 가장 흔하게 볼 수 있는 유형이다. 수표와 비트코인 트랜잭션을 비교하자면, 비트코인 주소는 수령인을 뜻하고 "~에게 지불"이라고 적혀있는 문구에서 공란에 적는 부분이라고 생각하면 이해하기 쉽다. 수표에서는 수령인이 은행 계좌 보유자이거나 기업, 기관, 심지어는 현금 자체가 될 수 있다. 수표는 계좌를 명시할 필요가 없고 수령처를 추상적으로 써도 되기 때문에 매우 유연한 지불수단이다. 비트코인 트랜잭션은 이와 유사하게 추상적인 비트코인 주소를 사용하기 때문에 매우 유연한 지불수단이다. 비트코인 주소는 프라이빗/공개키 쌍의 소유주를 나타내거나, <<p2sh>> 에서 보게 될 지불 스크립트와 같은 다른 대상을 나타낼 수도 있다. 지금부터는 쉬운 개념인, 공개키를 표현하는 수단이자 공개키에서 생성된 결과물인 비트코인 주소에 대해서 살펴보도록 하자.
...
((("addresses", "algorithms used to create")))비트코인 주소는 단방향 암호화 해싱을 사용하여 공개키를 생성한다. "해싱 알고리즘" 또는 단순히 "해시 알고리즘"은 임의 크기의 입력값으로부터 지문 또는 "해시"값을 생성해 내는 단방향 함수이다. 암호화 해시 함수는 비트코인에서 광범위하게 사용된다: 비트코인 주소, 스크립트 주소 및 작업증명 알고리즘 채굴. 공개키로부터 비트코인 주소를 생성하는데 사용되는 알고리즘은 '보안 해시 알고리즘 (SHA)' 및 RACE Integrity Primitives Evaluation Message Digest (RIPEMD), 구체적으로는 SHA256과 RIPEMD160 이다.
...
공개키 _K_ 로부터 SHA256 해시를 산출하고 그 결과값의 RIPEMD160 해시를 산출하면 160 비트 (20 바이트)의 숫자가 생성된다:
...
여기서 _K_ 는 공개키이고 _A_ 는 생성된 비트코인 주소이다.
...
비트코인 주소는 공개키와 동일하지 _않다_ . 비트코인 주소는 단방향 함수를 사용하여 공개키로부터 생성된다.
====
...
비트코인 주소는 거의 항상 "Base58Check"로 인코딩 되며 (<<base58>> 참고), Base58Check는 58개의 문자 (Base58 숫자체계)와 체크섬을 이용해서 가독성을 높이고 모호함을 방지하며 주소 변환 및 입력 오류를 방지한다. 또한 Base58Check는 비트코인에서 여러 다양한 방법으로 활용되며, 비트코인 주소, 프라이빗키, 암호화된 키, 스크립트 해시 등을 사용자가 읽고 정확하게 숫자를 입력할 수 있게 해준다. 이제 Base58Check 인코딩 및 디코딩 메커니즘과 그에 따른 표현법에 대해 알아보도록 하자. <<pubkey_to_address>> 는 공개키를 비트코인 주소로 변환하는 과정이 도식화되어 있다.

[[pubkey_to_address]]
// .Public key to bitcoin address: conversion of a public key into a bitcoin address
공개키에서 비트코인 주소까지: 공개키가 비트코인 주소가 되는 과정
